...
# The log transformation has improved adj-R2 value compared to sqrt and inverse
...
# Individual T-test Hypothesis considering market region 'africa' as base variable 
# and rest as dummy variable with 95% confidence level
# marketEMEA & marketUS > 0.05 are not significant - reject NUll hyp i.e. averages = 0
# the sales rate averages for the africa sales rate and the marketEMEA are not significantly different.
# the sales rate averages for the africa sales rate and the marketUS are not significantly diffrent.
# marketEU, marketLATAM, marketAPAC & marketCanada < 0.05 accept NUll Hyp i.e. averages not euqal to zero.
# averages sales rate of EU, LATAM, APAC, Canada and Africa are not same so, these 4 groups are
# significant and co-eff of these variables are not zero.
...
# As we are considering to use Market(Continents) and region removing 
# rest of the coulmns
...
train1.data=subset(train.data ,select= vifcoulmnnames)
